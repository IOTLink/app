...
func (t *EventSender) invoke(stub shim.ChaincodeStubInterface) pb.Response {
	fmt.Println(" in  func invoke\n")
	_, args := stub.GetFunctionAndParameters()
	if len(args) != 2 {
		fmt.Println("GetFunctionAndParameters","Incorrect number of arguments. Expecting 2")
	...
	//err = stub.SetEvent("myevent", []byte("myevent"))
	err = stub.SetEvent(eventName, []byte(tosend))
	if err != nil {
		fmt.Println("SetEvent",err.Error())
		return shim.Error(err.Error())
	}
