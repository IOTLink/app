...
func (coin *Hydruscoin) registerAccount(store Store, args []string) (error) {
	addr := args[1]
	...
	if tmpaccount, err := store.GetAccount(addr); err == nil && tmpaccount != nil && tmpaccount.Addr == addr {
		...
		fmt.Println("Hydruscoin account(%s) already registered.")
	...
	if err := store.PutAccount(account); err != nil {
		...
		fmt.Println("store.PutAccount return error: ", account, err)
		return  err
	}
