...
func main() {
	...
	userNameX := GenerateRandomID()
	_,_,err = myca.RegisterAndEnrollUser(userNameX,"userXW", "org1.department1")
	if err != nil {
		fmt.Println("RegisterAndEnrollUser FAILT",err)
		return
	} else {
		fmt.Println("RegisterAndEnrollUser User",userNameX)
	}
	...
	chaincodeName := "myutxo_vx9"

	/*
	err = server.InitAsset("mychannel", "github.com/hydrusio/hydruscoin", chaincodeName,  "v1", "Key", "Payload")
	if err != nil {
